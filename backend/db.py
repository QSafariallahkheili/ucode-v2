...
from models import MapOrientation
...
def update_project_starting_orientation(
    conn_pool: ConnectionPool, projectId: str, startingOrientation: MapOrientation
):
    with conn_pool.connection() as connection:

        insert_query_starting_Orientation = f"""
      UPDATE project
      SET starting_position = '{json.dumps(dataclasses.asdict(startingOrientation))}'
      WHERE project_id = '{projectId}';
  """
        connection.execute(insert_query_starting_Orientation)
        return "fine"
...
def add_comment(userId, projectId, comment, lng, lat, questId, routeId):
  ...
  """
    ...
    if questId != "NULL" and int(questId) > 0:
...
def update_voting_status(comment_id, user_id, action):
  ...
  """
    ...
    ):  # TH# if there is no voting up to now, set the voting status to "undefined"
...
def like_comment(commentid):
    ...
    cursor = connection.cursor()
    add_like_query = (
        f""" UPDATE comment SET likes = likes + 1 where id = {commentid};"""
    )
...
def dislike_comment(commentid):
    ...
    cursor = connection.cursor()
    add_dislike_query = (
        f""" UPDATE comment SET dislikes = dislikes + 1 where id = {commentid};"""
    )
...
def generate_zebra_crossing_table(projectId):
    connection = connect()
    cursor = connection.cursor()
    insert_zebra_crossing_query = f"""with a as (
    select ST_collect(geom) as geom from sidewalk where st_isvalid(geom) and project_id='{projectId}'
    ), b as (
        select st_union(geom) as geom from driving_lane_polygon where project_id='{projectId}'
    ), c as (
        select ST_collect(geom) as geom from bike where st_isvalid(geom) and project_id='{projectId}'
    ), merged as (
        select geom from a union select geom from c
    ), dumps as (
        select (st_dump(st_intersection(merged.geom, b.geom))).geom as geom from merged, b where st_intersects(merged.geom, b.geom)
    ), detected as (
        select ROW_NUMBER() OVER (ORDER BY dumps.geom) AS gid, dumps.geom from dumps, traffic_signal 
        where st_length(dumps.geom::geography)<14 and st_length(dumps.geom::geography)>2
        and ST_DWithin(dumps.geom::geography, traffic_signal.geom::geography, 4) 
        and traffic_signal.project_id='{projectId}'
        group by dumps.geom
    ), firstoffset as (
        select detected.gid, st_transform(ST_OffsetCurve(st_transform(ST_SetSRID(geom, 4326), 26986), 1),4326) as geom  from detected
    ), secondoffset as (
        select detected.gid, st_transform(ST_OffsetCurve(st_transform(ST_SetSRID(geom, 4326), 26986), -1),4326) as geom  from detected
    ), snappedone as (
        select firstoffset.gid, st_setsrid((ST_DumpPoints(ST_AsText(ST_Segmentize(ST_SetSRID(geom, 4326)::geography, 0.9)))).geom,4326)  as geom from firstoffset
    ), snappedsecond as (
        select secondoffset.gid, st_setsrid((ST_DumpPoints(ST_AsText(ST_Segmentize(ST_SetSRID(geom, 4326)::geography, 0.9)))).geom,4326)  as geom from secondoffset
    ), generatedlines as (
        select snappedone.gid, st_makeline(snappedone.geom, st_closestpoint(snappedsecond.geom, snappedone.geom)) as geom from snappedone, snappedsecond where snappedone.gid = snappedsecond.gid
    ), stripes as (
        select st_buffer(geom::geography, 0.1, 'endcap=square') as geom from generatedlines where st_length(geom::geography)<1.99
    )
...
def setup_new_project(data: ProjectInformation):
    # gets the project configuration data and puts them into the database
   ...
   insert into project (project_id, project_name, bbox, project_type, project_goal) values ('{project_id}','{project_name}', '{bbox}','{project_type}','{project_goal}')
   on conflict (project_id) do nothing;
  
  """
    # print(set_new_project)
    cursor.execute(set_new_project)
    connection.commit()
    cursor.close()
    connection.close()
    return data
